...
coef!(model::StatisticalModel, parameter) = 
  error("Method coef!(model::$(typeof(model)), parameter) must be implemented for $(typeof(model)).")
rand(model::StatisticalModel, T::Int) = 
  error("Method rand(model::$(typeof(model)),T::Int) must be implemented for $(typeof(model)).")
loglikelihood(model::StatisticalModel, data) = 
  error("Method loglikelihood(model::$(typeof(model)),data,parameter) must be implemented for $(typeof(model)).")


#return the dimension of the parameter: used to provide a random starting value when calling mle()
dim(model::StatisticalModel) = 
  error("If you want to use the default MLE method on a $(typeof(model)), you must define dim(model::$(typeof(model))).")
...
function mle_nojac(model::StatisticalModel,data,thetai::Array{Float64,1}=rand(dim(model)),L=15000)
  ...
  df = Optim.OnceDifferentiable(ff, thetai; autodiff = :forward);
  ret = Optim.optimize(df, thetai, ConjugateGradient(), iterations=L)
...
function mle_jac(model::StatisticalModel,data,thetai::Array{Float64,1}=rand(dim(model)),L=15000)
  ...
  df = Optim.TwiceDifferentiable(ff, fj!, ffj!, thetai)  
  ret = Optim.optimize(df, thetai, ConjugateGradient(), iterations=L)
